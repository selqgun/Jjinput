 ...
 *控件功能:
 *1.两种基础样式,(一行n个,一行一个)
 *2.可以通过传入具体json或ajax调url的方式设置待选项
 *3.以缓存方式记录ajax已查询过的结果,不必重复查询,节省资源
 *4.可以设置初始已选项.
 *5.可以设置选择项个数,控件宽度,下拉列表高度
 *6.可自定义可选项样式,增加图片等.
 *7.支持插入用户手工输入的自主数据项
 *8.已选项选择后该选项是否置灰.
 *9.控件自我销毁功能(待改进).
 *10.增加可选择扩展的字段(除id之外的额外属性值),例如:可以得到某用户的id,account,age等多个字段.
 *11.删除已选项时执行的函数,返回false时不执行删除操作,返回true可以删除
 *12.超高兼容性,在ie6上也能完美运行
 */
